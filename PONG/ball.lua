...
--[[
	...
	on whether their rectangles have overlap.
]]
function Ball:collides(paddle)
	--first check to see if the left edge is farther to the right than
	--the right edge of the paddle
	if self.x > paddle.x + paddle.width or paddle.x > self.x +self.width then
		return false
	end

	--now check to see if the bottom edge of either is higher than 
	--the top edge of the other, 
	if self.y > paddle.y + paddle.height or paddle.y > self.y + self.height then
		return false
	end

	--if the above arent true then they are overlapping
	return true
end
--[[
